...
awk -v search=$1 '{if ($1==search) print $2;}' /etc/EasyLifeNetworks/EasyLifeNetworks.conf
        ;;
...
# Call #9
NEIGHBORHOOD) if [ -f "/tmp/EasyLifeNetworks-neighborhood.txt" ];
                then
                 cat /tmp/EasyLifeNetworks-neighborhood.txt
                else
                 echo "0"
                fi
        ;;
